...
The data is provided via a static JSON file that should be loaded using Angular's HttpClient. The JSON file is located at `assets/db/movie-data.json` (you need to copy it from the `db` folder to `src/assets/db/`).

The JSON file contains three main sections:

- `movies` - An array of movie objects
- `genres` - An array of genre strings
- `actors` - An array of actor objects

### Accessing the Data

Create a service (e.g., `MovieService`) that uses Angular's `HttpClient` to load the data from the static JSON file:
...
export class MovieService {
  ...
  loadData() {
    return this.http.get<MovieData>(this.dataUrl);
  }
}
```

**Important**: Since we're using a static JSON file, create/update/delete operations should be handled in-memory only. The service should:
1. Load the JSON file once on initialization
2. Store the data in memory (e.g., using BehaviorSubject)
3. Perform all CRUD operations on the in-memory data
4. Note: Changes will not persist after page refresh (this is expected behavior for this exercise)
...
- If an actor is in the cast of a movie, it is not guaranteed that the actor will have an entry in the `actors` array
- If a movie is in the notable works of an actor, it is not guaranteed that the movie will have an entry in the `movies` array
- If we need to find a specific actor, we can search by the `name` field in the actors array
